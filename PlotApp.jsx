  ...
  return (
    <div style={{ height:"100%", overflowY:"auto", background:C.bg, paddingBottom:12 }}>
      <div style={{ padding:"12px 20px 0", position:"sticky", top:0, background:C.bg, zIndex:10 }}>
        ...
        <div style={{ display:"inline-flex", alignItems:"center", gap:5, background:C.purpleLight, borderRadius:20, padding:"5px 12px", marginBottom:14, fontSize:12, fontWeight:700, color:C.purple }}>{getAuraTag(userProfile)}</div>
    ...
    <div style={{ height:"100%", overflowY:"auto", background:C.bg }}>
      ...
      <div style={{ padding:"20px" }}>
        ...
        <div style={{ background:C.greenLight, borderRadius:20, padding:"16px", marginBottom:14, border:"1px solid #86EFAC" }}>
          ...
          {(reasons.length > 0
            ? reasons
            : ["You both like social, low pressure vibes","Similar energy levels","Group size fits your preference"]
          ).map((r,i)=>(
  ...
  return (
    <div style={{ height:"100%", display:"flex", flexDirection:"column", background:C.bg, padding:"40px 24px 30px" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        <button onClick={onContinue} style={{ padding:"16px", borderRadius:30, background:"#1C1917", color:"white", fontWeight:700, fontSize:16, border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}>
          <span style={{fontSize:20}}></span> Continue with Apple
        </button>
        <button onClick={onContinue} style={{ padding:"16px", borderRadius:30, background:"white", color:C.text, fontWeight:700, fontSize:16, border:`1px solid ${C.border}`, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}>
...
function OnboardingScreen({ onFinish }) {
  ...
  return (
    <div style={{ height:"100%", display:"flex", flexDirection:"column", background:C.bg, padding:"30px 24px" }}>
      ...
      <button onClick={() => onFinish(name || "Explorer")} style={{ padding:"18px", borderRadius:30, background:C.text, color:"white", fontWeight:800, fontSize:17, border:"none", cursor:"pointer", opacity:!name?0.5:1 }} disabled={!name}>
        Next
      </button>
    </div>
  );
    ...
    <div style={{
      ...
      <div style={{ padding:"20px 22px", flex:1 }}>
        ...
        <div style={{
          ...
          lineHeight:1.25, marginBottom:28, letterSpacing:-0.4,
        }}>
          {q.question}
        </div>
          ...
          {q.options.map(opt => (
            <button
              ...
              onMouseEnter={e => {
                ...
                e.currentTarget.style.boxShadow = `0 3px 14px rgba(255,104,69,0.18)`;
              }}
              onMouseLeave={e => {
            ...
            >
              <span style={{
                ...
                background:"#F5F2EC", borderRadius:12, flexShrink:0,
              }}>
                {opt.emoji}
              </span>
              <span style={{
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"flex-start", minHeight:"100vh", padding:"20px 16px 24px", background:"linear-gradient(135deg, #EDE8FC 0%, #E6F0FF 50%, #FDE8E0 100%)", fontFamily:"-apple-system, 'SF Pro Display', BlinkMacSystemFont, sans-serif" }}>
      ...
      <div style={{ width:375, background:C.bg, borderRadius:50, overflow:"hidden", boxShadow:"0 40px 80px rgba(0,0,0,0.22), 0 0 0 10px #1C1917, 0 0 0 12px #2E2E2E", display:"flex", flexDirection:"column", height:800 }}>
          ...
          {authState === "authenticated" && (
            !onboardingDone
              ...
              : <>
                  ...
                  {screen === "aura" && <AuraScreen energy={energy} setEnergy={setEnergy} intent={intent} setIntent={setIntent} mode={socialMode} setMode={setSocialMode} onFind={findPeople} loading={aiLoading} results={aiResults}/>}
        ...
        {authState === "authenticated" && onboardingDone && screen !== "event" && (
